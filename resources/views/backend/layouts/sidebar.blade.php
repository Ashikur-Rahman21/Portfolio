<!-- Sidebar Start -->
<div class="sidebar pe-4 pb-3">
    <nav class="navbar bg-light navbar-light">
        <a href="index.html" class="navbar-brand mx-4 mb-3">
            <h3 class="text-primary"><i class="fa fa-hashtag me-2"></i>ASHIK</h3>
        </a>
        <div class="d-flex align-items-center ms-4 mb-4">
            <div class="position-relative">
                <img class="rounded-circle" src="img/user.jpg" alt="" style="width: 40px; height: 40px;">
                <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
            </div>
            <div class="ms-3">
                <h6 class="mb-0">Ashikur Rahman</h6>
                <span>Admin</span>
            </div>
        </div>
        <div class="navbar-nav w-100">
            <a href="index.html" class="nav-item nav-link "><i class="fa fa-tachometer-alt me-2"></i>Dashboard</a>
            <a href="{{ route('hero-section.index') }}" class="nav-item nav-link "><i class="fa fa-tachometer-alt me-2"></i>Hero Section</a>
            <a href="{{ route('about-section.index') }}" class="nav-item nav-link "><i class="fa fa-tachometer-alt me-2"></i>About Section</a>
            <a href="{{ route('skill-section.index') }}" class="nav-item nav-link "><i class="fa fa-tachometer-alt me-2"></i>Skill Section</a>
            <a href="{{ route('education-section.index') }}" class="nav-item nav-link "><i class="fa fa-tachometer-alt me-2"></i>Education Section</a>            
            <a href="{{ route('experience-section.index') }}" class="nav-item nav-link"><i class="fa fa-tachometer-alt me-2"></i>Experience Section</a>            
            <a href="{{ route('service-section.index') }}" class="nav-item nav-link"><i class="fa fa-tachometer-alt me-2"></i>Service Section</a>            
            <a href="{{ route('portfolio-section.index') }}" class="nav-item nav-link"><i class="fa fa-tachometer-alt me-2"></i>Portfolio Section</a>            
        </div>
    </nav>
</div>
<!-- Sidebar End -->